...
Hi, I'm Eric Terry. I'm a biologist by training (PhD, UCSB; postdoc in aging, WashU), now working on agentic AI for science from a training and evaluation perspective. My background is in experimental design, not software engineering. What I bring to AI research is the same thing I brought to biology: asking really interesting questions and figuring out how to get the answers. In particular, I'm now asking "how can we teach ai think to better?"
...
The case study provides mechanistic evidence that internal states can diverge from output: a linear probe on hidden states predicts tool-use behavior at 99.4% accuracy, and the learned activation direction is nearly orthogonal to the difficulty direction (87.4 degrees) — meaning "this is hard" and "I want the tool" are geometrically separable internal signals. See the [subvocal desire detection](https://github.com/sixel-et/subvocal-desire) case study for full results.

## Related work

The closest neighbors: **Process reward models** (Lightman et al., 2023) reward intermediate reasoning steps, but source the reward signal from automated verification — our approach sources it from expert interaction traces, which encode *which step was missing*, not just whether the final answer was right. **Constitutional AI** (Bai et al., 2022) uses principles to guide behavior; our scaffold test is structurally similar, but the principles are extracted from observed failures rather than written a priori — the interaction trace generates the constitution. **Kadavath et al. (2022)** ("Language Models (Mostly) Know What They Know") established that models have calibration knowledge they don't always deploy; our capability/activation gap is a specific, reproducible instance of this finding with a concrete training target. The sycophancy literature (Sharma et al., 2023) documents how standard RLHF actively trains against epistemic caution — the opposite of what this work tries to install.
